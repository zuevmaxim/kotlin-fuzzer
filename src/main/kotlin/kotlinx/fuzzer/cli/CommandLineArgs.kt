...
class CommandLineArgs(parser: ArgParser) {
    ...
    private val classpath by parser.option(ArgType.String, description = "Target ClassPath (delimited with colon)").required().delimiter(DELIMITER)
    private val packages by parser.option(ArgType.String, description = "Target packages (delimited with colon)").required().delimiter(DELIMITER)
    private val maxTaskQueueSize by parser.option(ArgType.Int, description = "Maximum number of tasks in working queue. Use it to controll memory usage.")
